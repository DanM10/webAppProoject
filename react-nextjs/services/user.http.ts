import {noticia, report, report2, users} from "../interfaces";
...
export async function noticiaHttp():Promise<noticia[]>{
    const url = `http://localhost:3030/noticia`
    const resp = await fetch(url)
    return (await resp.json()) as noticia[]
}